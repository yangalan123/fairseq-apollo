...
@register_task('transformer_lra')
class TransformerLRATask(FairseqTask):
    """
    ...
    def setup_task(cls, args, **kwargs):
        ...
        data_dict = cls.load_dictionary(
            args,
            os.path.join(args.data, 'src-bin', 'dict.txt'),
            source=True,
        )
        ...
        if not args.regression_target:
            ...
            label_dict = cls.load_dictionary(
                args,
                os.path.join(args.data, 'label-bin', 'dict.txt'),
                source=False,
            )
            logger.info('[label] dictionary: {} types'.format(len(label_dict)))
        else:
            label_dict = data_dict
        return TransformerLRATask(args, data_dict, label_dict, cls_idx)
